...
validation_dataset = NflImageStacks(
    nfl_dir=valid_ds,
    num_frames=NUM_FRAMES, size=(512,512),
...
validation_loader = DataLoader(validation_dataset, batch_size=BS, shuffle=False, num_workers=BS)
...
if True:
    ...
    with torch.no_grad():
        for epoch in range(EPOCHS):
            model_path = os.path.join(model_dir,"seg_model_{}.pth".format(epoch))
            seg_model.load_state_dict(torch.load(model_path, map_location=lambda storage, loc: storage))
            ...
            for batch_idx, sample in val_loop:
                ...
                val_loop.set_postfix(losses="rot: {:.6f}".format(loss_add.item()))

            with open(os.path.join(path,"val_log"), "a") as log:
                log.write("validation: {} {:.6f}\n".format(epoch, epoch_loss/cnt_sample ))